...
var Tutorial = function (game) {
    ...
    this.BURN_BAR_INCREMENT_CHANGE = -0.05;
    this.BURN_METER_CONSTANT_CHANGE = -0.01;
    var playerBlinking = false; // checked to see if we're already blinking and to reset function if needed
};
...
Tutorial.prototype = {
    ...
    isPlayerReady: function () {
        if (this.instructions.length === 0) {
            ...
            var tutorialText = game.add.text(game.world.width / 2, game.world.height / 2, "Try to burn everything", style);
            tutorialText.alpha = 0;
            tutorialText.anchor.set(0.5);
            tutorialText.setShadow(3, 3, 'rgba(0,0,0,0.5)', 0);
            game.add.tween(tutorialText).to({alpha: 1}, 700, Phaser.Easing.Linear.None, true);
            game.time.events.add(2000, function() {
                game.add.tween(tutorialText). to({y: 200}, 1500, Phaser.Easing.Linear.None, true);
                game.add.tween(tutorialText).to({alpha: 0}, 1500, Phaser.Easing.Linear.None, true);}, this);
            ...
            game.time.events.loop(Phaser.Timer.SECOND, this.playerBurnMeterConstantChange, this);

            // Play event
            var time = Phaser.Timer.SECOND * 10;
    ...
    playerBlink: function () {
        for (let i = 0; i < 3; ++i) {
            game.add.tween(this.player).to({alpha: 0.1}, 300, Phaser.Easing.Linear.None, true)
